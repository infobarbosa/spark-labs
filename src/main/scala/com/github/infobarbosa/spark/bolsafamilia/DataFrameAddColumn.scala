...
import org.apache.spark.sql.functions.{col, regexp_replace, lit}

object DataFrameAddColumn extends App{
  //Exemplo de adicao de colunas num dataframe atraves do metodo withColumn
  def main() {
    ...
    val competencia_valores =
      rawPayments
        .withColumn("vl_parcela", regexp_replace(col("valor_parcela"), ",", ".")
          .cast("float"))
      .withColumn("nova_coluna", lit(1))
      .select("competencia", "vl_parcela", "nova_coluna")
